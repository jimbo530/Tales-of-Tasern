/**
 * add-usdglo-to-ddd-reactor.js — Add USDGLO pool to existing DDD reactor
 * Run: node add-usdglo-to-ddd-reactor.js
 */
...
async function main() {
  ...
  const gloC = new ethers.Contract(USDGLO, ERC20_ABI, wallet);

  const dddBal = await dddC.balanceOf(wallet.address);
  const gloBal = await gloC.balanceOf(wallet.address);
  const startCount = await reactor.poolCount();

  log('Wallet: ' + wallet.address);
  log('DDD reactor: ' + REACTOR + ' (' + startCount.toString() + ' pools)');
  log('USDGLO: ' + ethers.formatEther(gloBal) + ' ($' + Number(ethers.formatEther(gloBal)).toFixed(2) + ')');
  log('DDD: ' + Number(ethers.formatEther(dddBal)).toLocaleString());

  if (gloBal === 0n) { log('ERROR: No USDGLO'); process.exit(1); }

  // Match DDD to USDGLO value
  const gloUsdVal = Number(ethers.formatEther(gloBal)) * USDGLO_USD;
  const dddNeeded = gloUsdVal / DDD_USD;
  ...
  const gloAllow = await gloC.allowance(wallet.address, NPM);
  if (gloAllow < gloBal) {
    log('Approving USDGLO...');
    const tx = await gloC.approve(NPM, ethers.MaxUint256, await gas());
    await tx.wait();
  }
  log('Approvals done');
  ...
  let poolAddr = await factory.getPool(DDD, USDGLO, FEE);

  if (poolAddr === ZERO) {
    log('Creating V3 1% pool...');
    const tx = await factory.createPool(DDD, USDGLO, FEE, await gas());
    await tx.wait();
    poolAddr = await factory.getPool(DDD, USDGLO, FEE);
    log('Pool: ' + poolAddr);
  } else {
  ...
  try {
    ...
    if (slot0[0] === 0n) {
      const dddIsT0 = DDD.toLowerCase() < USDGLO.toLowerCase();
      const usd0 = dddIsT0 ? DDD_USD : USDGLO_USD;
      const dec0 = dddIsT0 ? DDD_DEC : USDGLO_DEC;
      const usd1 = dddIsT0 ? USDGLO_USD : DDD_USD;
      const dec1 = dddIsT0 ? USDGLO_DEC : USDGLO_DEC;
  ...
  log('\n=== MINT ===');
  const dddIsT0 = DDD.toLowerCase() < USDGLO.toLowerCase();
  const token0 = dddIsT0 ? DDD : USDGLO;
  const token1 = dddIsT0 ? USDGLO : DDD;
  const amount0 = dddIsT0 ? dddUse : gloBal;
  const amount1 = dddIsT0 ? gloBal : dddUse;

  const deadline = Math.floor(Date.now() / 1000) + 600;
  log('Minting DDD/USDGLO position...');
  ...
  log('USDGLO pool added — position #' + tokenId.toString());
